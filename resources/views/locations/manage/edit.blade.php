...
<head>
    ...
    <title>Редагування локації - Історія авіації на Закарпатті</title>
    ...
    <header class="page-header">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h1><i class="fas fa-edit me-2"></i>Редагування локації</h1>
                    <p class="lead">{{ $location->name }} ({{ $location->type }})</p>
                </div>
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <form action="{{ route('locations.manage.update', $location->id) }}" method="POST" id="editLocationForm">
                            @csrf
                            @method('PUT')

                            <div class="mb-4">
                                <label for="name" class="form-label"><i class="fas fa-map-pin me-1"></i> Назва локації</label>
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $location->name) }}" required placeholder="Введіть назву локації">
                                @error('name')
                            ...
                            <div class="mb-4">
                                ...
                                <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                                    <option value="" disabled>Виберіть тип локації</option>
                                    <option value="Аеропорт" {{ old('type', $location->type) == 'Аеропорт' ? 'selected' : '' }}>Аеропорт</option>
                                    <option value="Аеродром" {{ old('type', $location->type) == 'Аеродром' ? 'selected' : '' }}>Аеродром</option>
                                    <option value="Музей" {{ old('type', $location->type) == 'Музей' ? 'selected' : '' }}>Музей</option>
                                    <option value="Пам’ятник" {{ old('type', $location->type) == 'Пам’ятник' ? 'selected' : '' }}>Пам’ятник</option>
                                    <option value="Інше" {{ old('type', $location->type) == 'Інше' ? 'selected' : '' }}>Інше</option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-4">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="5" placeholder="Опис локації (необов'язково)">{{ old('description', $location->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                        ...
                                        <div class="mt-3">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    ...
                                                    <input type="text" class="form-control @error('latitude') is-invalid @enderror" id="latitude" name="latitude" value="{{ old('latitude', $location->latitude) }}" readonly>
                                                    @error('latitude')
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                                <div class="col-md-6">
                                                    ...
                                                    <input type="text" class="form-control @error('longitude') is-invalid @enderror" id="longitude" name="longitude" value="{{ old('longitude', $location->longitude) }}" readonly>
                                                    @error('longitude')
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                            ...
                                            <small class="text-muted mt-2 d-block">
                                                ...
                                                Натисніть на карту, щоб змінити розташування локації. Координати оновляться автоматично.
                                            </small>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="card mb-4 sticky-top" style="top: 20px;">
                    ...
                    <div class="card-body">
                        <div class="d-grid gap-3">
                            <button type="submit" form="editLocationForm" class="btn btn-primary btn-lg">
                                <i class="fas fa-save me-2"></i>Зберегти зміни
                            </button>
                            <a href="{{ route('filter.location', ['location_id' => $location->id]) }}" class="btn btn-info" target="_blank">
                    ...
                    <div class="card-body">
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <span><i class="fas fa-calendar-alt me-2"></i>Пов'язані події:</span>
                                <span class="badge bg-primary rounded-pill">{{ $location->events->count() }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
        ...
        function placeMarker(location) {
            ...
            marker.bindPopup(`
                <div style="padding: 5px;">
                    <h6 style="margin: 0 0 5px 0; color: #0d47a1;">{{ $location->name }}</h6>
