...
<head>
    ...
    <title>Редагування користувача - Історія авіації на Закарпатті</title>
    ...
    <header class="page-header">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h1><i class="fas fa-user-edit me-2"></i>Редагування користувача</h1>
                    <p class="lead">{{ $user->name }} ({{ $user->email }})</p>
                </div>
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <form action="{{ route('users.manage.update', $user->id) }}" method="POST" id="editUserForm">
                            @csrf
                            @method('PUT')

                            <div class="mb-4">
                                <label for="name" class="form-label"><i class="fas fa-user me-1"></i> Ім'я користувача</label>
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $user->name) }}" required placeholder="Введіть ім'я користувача">
                                @error('name')
                            ...
                            <div class="mb-4">
                                ...
                                <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $user->email) }}" required placeholder="Введіть email">
                                @error('email')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                <label for="password" class="form-label"><i class="fas fa-lock me-1"></i> Новий пароль</label>
                                <input type="password" class="form-control @error('password') is-invalid @enderror" id="password" name="password" placeholder="Залиште порожнім, щоб не змінювати">
                                <div class="form-text">Пароль повинен містити не менше 8 символів. Залиште порожнім, щоб зберегти поточний пароль.</div>
                                @error('password')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="password" class="form-control" id="password_confirmation" name="password_confirmation" placeholder="Підтвердіть новий пароль">
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="card mb-4 sticky-top" style="top: 20px;">
                    ...
                    <div class="card-body">
                        <div class="d-grid gap-3">
                            <button type="submit" form="editUserForm" class="btn btn-primary btn-lg">
                    ...
                    <div class="card-body">
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                ...
                                <span class="badge bg-primary rounded-pill">{{ $user->events->count() }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
